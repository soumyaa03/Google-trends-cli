...
func main() {
	...
	fmt.Println("\n Below are the all Google Search Trends for Today !")
	fmt.Println("----------------------------------------------------------")
	...
	for i := range r.Channel.ItemList {
		...
		// we can use another loop for all news items , but for simplicity while printing we print only first NewsItems
		fmt.Println("HeadLine:", r.Channel.ItemList[i].NewsItems[0].Headline)
		fmt.Println("Link to the Article:", r.Channel.ItemList[i].NewsItems[0].HeadlineLink)
		fmt.Println("---------------------------------------------")
